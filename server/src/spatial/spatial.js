 ...
 * Uses adaptive step size for resolution-aware performance.
 */
...
/**
 * Downsample coordinates for faster polygon detection.
 */
...
/**
 * OPTIMIZED Paper.io-Style Geometry Engine (Server-side).
 * Uses coordinate downsampling + single-pass self-intersection detection.
 */
export function extractEnclosedTerritoryGeometry(rawCoords, options = {}) {
  ...
  // 1. Direct unkink on the full closed loop
  ...
  for (let i = 0; i < n - 1; i++) {
    ...
    for (let j = i + 2; j < n - 1; j++) {
      ...
      if (pt) {
        intersections.push({ segI: i, segJ: j, point: [pt[0] / cosLat, pt[1]] });
      }
    }
  }
...
/**
 * FAST live loop enclosure detection — O(n) per call.
 */
export function detectLatestLoopEnclosure(rawCoords, processedLoopKeys = new Set(), resolution = H3_DEFAULT_RESOLUTION) {
  ...
  try {
    ...
    try {
      ...
      for (const h of hexes) newInterior.push(h);
    } catch (e) {}

    return {
      newLoop: { loopKey, areaM2 },
...
/**
 * Calculates total route metrics including enclosed polygon area
 */
export function calculateWorkoutMetrics(gpsPoints, userWeightKg = 70) {
  ...
  for (let i = 1; i < gpsPoints.length; i++) {
    ...
    const lat1 = p1.latitude ?? p1[1];
    const lon1 = p1.longitude ?? p1[0];
    const lat2 = p2.latitude ?? p2[1];
    const lon2 = p2.longitude ?? p2[0];
    totalMeters += calculateDistanceMeters(lat1, lon1, lat2, lon2);
  }

  const startTime = gpsPoints[0].timestamp || Date.now() - gpsPoints.length * 3000;
  const endTime = gpsPoints[gpsPoints.length - 1].timestamp || Date.now();
  ...
  const MET = avgSpeedKmh > 8 ? 9.8 : avgSpeedKmh > 5 ? 7.0 : 3.8;
  const caloriesBurned = Math.round(MET * userWeightKg * hours);

  const territoryResult = extractEnclosedTerritoryCells(gpsPoints);
  ...
  return {
    ...
    caloriesBurned,
    cellsCovered: territoryResult.allCells,
